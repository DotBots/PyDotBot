...
you're working with one DotBot on the bench, or before the fleet is set up. For
...
Place the DotBot on the floor square and run the TUI. `-d` is the side length of
...
- **No counts in the TUI** - wrong `-p` port, or the DotBot can't see both base
