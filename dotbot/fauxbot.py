...
class FauxBotRegistry(type):
    def __iter__(cls):
        return iter(cls._registry)
    
class FauxBot(metaclass=FauxBotRegistry):
    
    __metaclass__ = FauxBotRegistry
    _registry = []
    ...
    def update(self):
        ...
        if self.controller=="MANUAL":
            self.pos_x, self.pos_y, self.theta = diff_drive_bot(pos_x_old, pos_y_old, theta_old, self.v_right, self.v_left)
        ...
        if (self.address == hex(payload.header.destination)[2:]):
            if (payload.payload_type == PayloadType.CMD_MOVE_RAW):
                ...
                for i in range(self.num_waypoints):
                    self.waypoints_x.append(int.from_bytes(decoded_frame[27+12*i:30+12*i], byteorder = "little"))
                    self.waypoints_y.append(int.from_bytes(decoded_frame[31+12*i:34+12*i], byteorder = "little"))
