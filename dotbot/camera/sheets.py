...
"""The printed ArUco sheets: what one looks like, and where it goes.
...
That derivation is what `marker_layout` returns and what the solve fits a
homography to, so the layout lives here beside the renderer that prints it:
a change to the page geometry moves both at once or neither.
"""
