...
import { dotbotStatuses, dotbotBadgeStatuses, XGOActionId } from "./utils/constants";
...
interface XGOItemProps {
  dotbot: DotBot;
  updateActive: (address: string) => void;
  publishCommand: PublishCommandFn;
}

export const XGOItem: React.FC<XGOItemProps> = ({ dotbot, updateActive, publishCommand }) => {
  ...
  const applyAction = async (action: number): Promise<void> => {
    log.info(`Applying XGO action ${action}`);
    await publishCommand(dotbot.address, dotbot.application, "xgo_action", { action });
  };
