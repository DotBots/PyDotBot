...
import { DotBot, GpsPosition, LH2Position, WsMessage } from "../types";

export const lh2_distance = (lh21: LH2Position, lh22: LH2Position): number => {
  return Math.sqrt(((lh21.x - lh22.x) ** 2) + ((lh21.y - lh22.y) ** 2));
};

export const gps_distance = (gps1: GpsPosition, gps2: GpsPosition): number => {
  const _gps1 = { lat: gps1.latitude, lon: gps1.longitude };
  const _gps2 = { lat: gps2.latitude, lon: gps2.longitude };
  return geodist(_gps1, _gps2, { exact: true, unit: 'meters' });
};

export const handleDotBotUpdate = (prevList: DotBot[], message: WsMessage): DotBot[] => {
  let changed = false;
  const nextList = prevList.map(bot => {
    if (bot.address !== message.data.address) { return bot; }

    let botChanged = false;
    let updated: DotBot = bot;
    ...
    if (
      message.data.lh2_position != null &&
      bot.lh2_position != null &&
      lh2_distance(bot.lh2_position, message.data.lh2_position) > lh2_distance_threshold
    ) {
      const newHistory = [...bot.position_history, message.data.lh2_position];
    ...
    if (
      ...
      bot.gps_position != null &&
      gps_distance(bot.gps_position, message.data.gps_position) > gps_distance_threshold
    ) {
      const newHistory = [...bot.position_history, message.data.gps_position];
