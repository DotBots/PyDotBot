...
class SCT:
    ...
    def input_read(self, ev):
        ...
        if ev < self.num_events and self.callback[event_name]:
            return self.callback[event_name]["check_input"](
                self.callback[event_name]["sup_data"]
            )
        return False
    ...
    def get_state_position(self, supervisor, state):
        position = self.sup_data_pos[
            supervisor
        ]  # Jump to the start position of the supervisor
    ...
    def make_transition(self, ev):
        ...
        for i in range(0, self.num_supervisors):
            if self.sup_events[i][
                ev
            ]:  # Check if the given event is part of this supervisor
                ...
                while num_transitions:
                    ...
                    if value == ev:
                        self.sup_current_state[i] = (
                            self.sup_data[position + 1] * 256
                        ) + (self.sup_data[position + 2])
                        break
                    position += 3
...
class SCTProb(SCT):
    ...
    def get_state_position_prob(self, supervisor, state):
        prob_position = self.sup_data_prob_pos[
            supervisor
        ]  # Jump to the start position of the supervisor
        for s in range(0, state):  # Keep iterating until the state is reached
            en = self.sup_data_prob[
                prob_position
            ]  # The number of transitions in the state
        ...
        for i in range(0, self.num_supervisors):
            ...
            while num_transitions:
                ...
                if self.ev_controllable[value] and self.sup_events[i][value]:
                    ev_disable[value] = (
                        0  # Transition with this event, do not disable it, just calculate its probability contribution
                    )
