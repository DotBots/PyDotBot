...
async def main() -> None:
    ...
    sct_path = os.getenv(
        "DOTBOT_SCT_PATH", "dotbot/examples/minimum_naming_game/models/supervisor.yaml"
    )
    ...
    async with rest_client(url, port, use_https) as client:
        ...
        try:
            ...
            while True:
                ...
                coords = [
                    [dotbot.lh2_position.x, dotbot.lh2_position.y] for dotbot in dotbots
                ]
                ...
                for dotbot in dotbots:
                    ...
                    neighbor_indices = tree.query_ball_point(
                        [dotbot.lh2_position.x, dotbot.lh2_position.y], r=COMM_RANGE
                    )
                    ...
                    if neighbors:
                        selected_neighbor = dotbot_controllers[
                            random.choice(neighbors).address
                        ]
                    ...
                    if dotbot.lh2_position.x + controller.vector[0] < 0:
                        point.y = controller.vector[1] * (
                            controller.vector[0] / MAX_SPEED
                        )
                        point.x = 0.0
                    if dotbot.lh2_position.y + controller.vector[1] < 0:
                        point.x = controller.vector[0] * (
                            controller.vector[1] / MAX_SPEED
                        )
                        point.y = 0.0
                    ###
