...
import { INACTIVE_ADDRESS, LH2_DISTANCE_THRESHOLD, WEBSOCKET_URL } from "./constants"
import { DotBotAddressModel, DotBotModel, DotBotLH2Position, DotBotNotificationCommand } from "./models";
...
export default function useDotBots() {
  ...
  const [activeDotBot, setActiveDotBot] = useState<DotBotAddressModel>({ address: INACTIVE_ADDRESS });
  ...
  const fetchdotBots = useCallback(async () => {
    ...
    const active = await api.fetchActiveDotBotAddress().catch(error => console.log(error));
    if (active) {
  ...
  const onWsMessage = (ev: { data: string }) => {
    ...
    if (message.cmd === DotBotNotificationCommand.UPDATE && dotBots && dotBots.length > 0) {
      ...
      // DotBot to update
      const dotBot = newDotBots.find((dotBot) => dotBot.address === message.data.address);
      ...
      if (message.data.lh2_position) {
        ...
        if (dotBot.lh2_position && (dotBot.position_history.length === 0 || lh2_distance(dotBot.lh2_position, newPosition) > LH2_DISTANCE_THRESHOLD)) {
